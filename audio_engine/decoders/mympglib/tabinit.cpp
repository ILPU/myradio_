...
void __fastcall make_decode_tables(long scaleval)
{
    int i,j,k,kr/*,divv*/;
    real *costab,divv;
    int idx;
    ...
    for(i=0; i<5; i++)
    {
        ...
        costab = pnts[i];
        for(k=0; k<kr; k++)
            costab[k] = 1.0 / (2.0 * cos(M_PI * ((double) k * 2.0 + 1.0) / divv));
    }
...
//      decwin[idx+16] = decwin[idx] = (double) intwinbase[j] / 65536.0 * (double) scaleval;
            decwin[idx+16] = decwin[idx] = (double) intwinbase[j] * (double) scaleval / 65536.0;
