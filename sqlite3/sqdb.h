...
public:
    ...
    Exception(sqlite3* db, int errorCode, wstring *lastq);
...
#define CHECK(db, returnCode, lastquerys) \
  if ((returnCode) != SQLITE_OK) throw Exception(db, returnCode, lastquerys)
class RefCount
{
protected:
    RefCount();
...
{
public:
    Statement(sqlite3* db, sqlite3_stmt* stmt, wstring *lqs);
